...
class _SeatSelectionPageState extends State<SeatSelectionPage> {
    ...
    _seats = List.generate(
      ...
      (r) => List.generate(
        ...
        (s) {
          ...
          return _random.nextDouble() > 0.5 ? SeatStatus.available : SeatStatus.occupied;
        },
      ),
    );
  }
  ...
  bool _isSeatOccupied(int row, int col) =>
      _seats[row][col] == SeatStatus.occupied || _seats[row][col] == SeatStatus.aisle;
  ...
  void _toggleSeat(int row, int col) {
    ...
    if (state.filmTickets.isNotEmpty && _assignedSeats != null) {
      ...
      setState(() {
        if (idx >= 0) {
          ...
          if (currentBillet >= 0 && _canSelectSeatForBillet(row, col, currentBillet)) {
    ...
    setState(() {
      _seats[row][col] = current == SeatStatus.selected ? SeatStatus.available : SeatStatus.selected;
    });
  ...
  bool _canSelectSeatForBillet(int row, int col, int billetIndex) {
    final state = ReservationState.instance;
    if (billetIndex < 0 || billetIndex >= state.filmTickets.length) return false;
    final seatId = _seatId(row, col);
  ...
  bool _canSelectSeat(int row, int col) {
    ...
    final state = ReservationState.instance;
    if (!state.isEvent && state.filmTickets.isNotEmpty && _assignedSeats != null) {
      final seatId = _seatId(row, col);
    ...
    if (!state.isEvent && state.filmTitle == null) {
      return Center(
        child: Column(
          ...
          children: [
            const Text('Aucune réservation en cours.', style: TextStyle(color: AppTheme.textPrimary)),
            const SizedBox(height: 16),
            TextButton(onPressed: () => context.go(AppRouter.films), child: const Text('Voir les films')),
          ],
        ),
      );
    }

    if (state.filmTickets.isNotEmpty && (_assignedSeats == null || _assignedSeats!.length != state.filmTickets.length)) {
      _assignedSeats = List.filled(state.filmTickets.length, null);
    } else if (state.filmTickets.isEmpty) {
      _assignedSeats = null;
    }

    final currentBilletIndex = _assignedSeats != null ? _assignedSeats!.indexWhere((s) => s == null) : -1;
    final hasPerBilletMode = state.filmTickets.isNotEmpty && _assignedSeats != null;
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              SizedBox(
                                width: 20,
                                child: Text(_rows[r], style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                              ),
                              const SizedBox(width: 8),
                              Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  for (var c = 0; c < _seatsPerRow; c++) ...[
                                    _seatWidget(r, c),
                                    if (c < _seatsPerRow - 1) const SizedBox(width: 4),
                                  ],
                                ],
                              ),
                              const SizedBox(width: 8),
                  ...
                  children: [
                    ...
                    if (hasPerBilletMode && _assignedSeats != null) ...[
                      const Padding(
                        ...
                        child: Text('Sièges par billet', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                      ),
                      ...List.generate(state.filmTickets.length, (i) {
                        final seat = _assignedSeats![i];
                        final type = state.filmTickets[i].isVip ? 'VIP' : 'Normal';
                        return Padding(
                          padding: const EdgeInsets.only(bottom: 6),
                          child: Row(
                            children: [
                              Text('Billet ${i + 1} ($type): ', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                              Text(seat ?? '—', style: const TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w500)),
                            ],
                          ),
                        );
                      }),
                      ...
                      _recapRow(
                        'Sièges sélectionnés',
                        state.selectedSeats.isEmpty ? 'Aucun siège sélectionné' : state.selectedSeats.join(', '),
                      ),
                    const SizedBox(height: 8),
                    Text(
                    ...
                    Builder(
                      builder: (context) {
                        final ok = state.filmTickets.isNotEmpty
                            ? state.selectedSeats.length == state.filmTickets.length
                            : state.selectedSeats.isNotEmpty;
                        return SizedBox(
                          width: double.infinity,
                          child: FilledButton(
                            onPressed: ok ? () => context.go(AppRouter.paiement) : null,
  ...
  Widget _seatWidget(int row, int col) {
    ...
    if (status == SeatStatus.aisle) return const SizedBox(width: 28, height: 28);
    ...
    final assignedBillet = isSelected && _assignedSeats != null ? _assignedBilletIndex(_seatId(row, col)) + 1 : null;
    return GestureDetector(
      onTap: (isOccupied || isDisabled) ? null : () => _toggleSeat(row, col),
      child: Tooltip(
        message: assignedBillet != null ? 'Billet $assignedBillet' : (isVip ? 'Siège VIP' : 'Siège standard'),
        child: Container(
          width: 28,
          height: 28,
          decoration: BoxDecoration(
            color: isOccupied
                ...
                : isDisabled
                    ...
                    : (isSelected ? AppTheme.primaryRed : (isVip ? AppTheme.primaryRed.withValues(alpha: 0.25) : AppTheme.cardDark)),
